...
myaiproxy представляет собой FastAPI приложение, которое оперирует несколькими ключевыми понятиями:
...
```bash
docker build . -t myaiproxy:latest
```
...
docker run -p 8000:8000 -v ./config.yaml:/home/app/config.yaml myaiproxy:latest
```
